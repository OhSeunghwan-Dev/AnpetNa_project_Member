...
//     부모들 (nullable)
    ...
    @JoinColumn(name = "member_id",
        foreignKey = @ForeignKey(name = "fk_image_member"))
    ...
    @JoinColumn(name = "review_id",
            foreignKey = @ForeignKey(name = "fk_image_board"))
...
//     ====== 편의 메서드: 부모 연결은 한 번에 하나만 ======
    ...
    private void validateExactlyOneParent() {
        ...
        if (review != null) cnt++;
        if (cnt != 1) {
            throw new IllegalStateException("Image must be attached to exactly ONE parent (board|member|item).");
        }
    }

    // 정적 팩토리
    public static ImageEntity forBoard(String url, BoardEntity b, Integer order) {
    ...
    public static ImageEntity forMember(String url, MemberEntity m, Integer order) {
        ImageEntity img = new ImageEntity();
        img.setUrl(url);
        img.setSortOrder(order == null ? 0 : order);
    ...
    public static ImageEntity forItem(String url, ItemEntity i, Integer order) {
        ImageEntity img = new ImageEntity();
        img.setUrl(url);
        img.setSortOrder(order == null ? 0 : order);
        img.attachToItem(i);
    ...
    public static ImageEntity forReview(String url, ReviewEntity r, Integer order) {
        ImageEntity img = new ImageEntity();
        img.setUrl(url);
        img.setSortOrder(order == null ? 0 : order);
